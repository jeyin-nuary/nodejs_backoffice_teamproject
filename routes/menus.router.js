...
// 메뉴 생성
router.post('/stores/:storeId/menus', middleware, async (req, res) => {
  try {
    const { storeId } = req.params;
...
// 메뉴 수정
router.put('/stores/:storeId/menus/:menuId', middleware, async (req, res) => {
  try {
...
// 메뉴 삭제
router.delete('/stores/:storeId/menus/:menuId', middleware, async (req, res) => {
  try {
    const { storeId, menuId } = req.params;
    const store = await Stores.findOne({ where: { storeId } });
    if (!store) return res.status(404).json({ message: '가게가 존재하지 않습니다.' });
